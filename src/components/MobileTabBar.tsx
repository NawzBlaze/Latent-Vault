...
/**
 * Floating glass tab bar for coarse pointers. Rendered as a top-level element
 * (NOT inside any blurred/transformed ancestor) so `position: fixed` resolves
 * against the viewport — backdrop-filter on a parent would otherwise trap it.
 * Hidden entirely on desktop by CSS.
 */
      ...
      {NAV_LINKS.map((l) => {
        ...
        return (
          ...
          >
            ...
            <span className="tabbar-label">{l.label}</span>
          </Link>
        );
      })}
    </nav>
